...
--IF OBJECT_ID('CourseDescriptionChunks') IS NOT NULL DROP INDEX CourseDescriptionChunks;
...
/*
    ...
    COUNT(embedding) AS NonNullEmbeddings,
    VECTOR_DIMENSIONS(MAX(embedding)) AS Dimensions
FROM Chunks;

-- See which vector-related functions exist on your build
SELECT name 
FROM sys.objects 
WHERE name LIKE '%vector%' OR name LIKE '%VECTOR%';

-- Check VECTOR_DISTANCE is available
SELECT TOP 5
    CourseID,
    content,
    VECTOR_DISTANCE('cosine', embedding, (SELECT TOP 1 embedding FROM Chunks)) AS distance
FROM Chunks
ORDER BY distance ASC;

    execute procCreateVectorIndexOnChunks
*/
GO
